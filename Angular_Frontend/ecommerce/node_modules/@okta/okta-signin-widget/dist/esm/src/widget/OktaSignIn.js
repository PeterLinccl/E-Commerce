...
function createOktaSignIn(authClientConstructor, routerClassFactory) {
  return class OktaSignIn {
    constructor(options) {
      ...
      this.authClient = getAuthClientInstance(authClientConstructor, options); // validate authClient configuration against widget options
      ...
      this.Router = routerClassFactory(options); // Triggers the event up the chain so it is available to the consumers of the widget.

      this.Router.prototype.Events.listenTo.call(this, this.Router.prototype, 'all', this.trigger); // On the first afterRender event (usually when the Widget is ready) - emit a 'ready' event
